...
import datasets.fastmri.subsample as subsample
import datasets.fastmri.transforms as transforms
import datasets.fastmri.utils as utils
...
class FastMRIDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        inp = sample[0]/torch.max(sample[0])
        outp = sample[1]/torch.max(sample[1])
        input_img = torch.clip(inp, 0, 1)
        output_img = torch.clip(outp, 0, 1)
